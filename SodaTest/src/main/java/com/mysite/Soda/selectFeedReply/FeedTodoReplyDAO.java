...
public class FeedTodoReplyDAO {
	
	private final NamedParameterJdbcTemplate jdbcTemplate;
	
	RowMapper<FeedTodoReplyVO> rowMapper = (rs, rowNum) ->
		new FeedTodoReplyVO(
	...
	public List<FeedTodoReplyVO> projectID(int project_id) {
		String sql = "SELECT m.profile_image, r.contents, r.reply_date, m.name, fd.feed_id, r.reply_id, m.member_id "
				+ "FROM reply r JOIN member m ON r.member_id = m.member_id "
				+ "JOIN feed_todo fd ON fd.feed_id = r.feed_id "
				+ "JOIN feed f ON f.feed_id = fd.feed_id "
