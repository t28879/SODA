...
public class FeedWorkReplyDAO {
	
	private final NamedParameterJdbcTemplate jdbcTemplate;
	
	RowMapper<FeedWorkReplyVO> rowMapper = (rs, rowNum) ->
		new FeedWorkReplyVO(
	...
	public List<FeedWorkReplyVO> projectID(int project_id) {
		String sql = "SELECT m.profile_image, r.contents, r.reply_date, m.name, fw.feed_id, r.reply_id, m.member_id "
				+ "FROM reply r JOIN member m ON r.member_id = m.member_id "
				+ "JOIN feed_work fw ON fw.feed_id = r.feed_id "
